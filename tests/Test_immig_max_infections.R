...
test_immig_max_infections<-netsim(est, param, init, control)

##Outside of package
save(test_immig_max_infections, file = "~/EpiModelHIV_shamp_modeling/scenarios/test_immig_max_infections.rda")

##Run the Check


sum(test_immig_max_infections$epi$incid.L,na.rm = TRUE)
sum(test_immig_max_infections$epi$incid.H,na.rm = TRUE)
sum(test_immig_max_infections$epi$incid.FA,na.rm = TRUE)

table(test_immig_max_infections$attr[[1]]$inf.class)
